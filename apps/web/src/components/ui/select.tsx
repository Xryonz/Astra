...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      'flex h-10 w-full items-center justify-between border border-(--border-mid) bg-(--raised)/40 px-3 py-2 text-sm font-(family-name:--font-body)',
  ...
  >
    ...
    <SelectPrimitive.Icon asChild>
      <ChevronDown className="size-4 text-(--text-3) opacity-70" />
...
>(({ className, children, position = 'popper', ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ref={ref}
      className={cn(
        'relative z-50 max-h-96 min-w-[8rem] overflow-hidden border border-(--border-mid) bg-(--overlay) text-foreground shadow-2xl',
...
>(({ className, ...props }, ref) => (
  <SelectPrimitive.Label
    ref={ref}
    className={cn('px-2 py-1.5 font-mono text-[10px] uppercase tracking-wider text-(--text-3)', className)}
    {...props}
  />
))
