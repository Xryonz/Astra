...
- [~] M6 LiveKit Android (fatiado, valida cada slice com 2 contas):
  ...
  - [~] M6d screenshare (fatiado):
    - [~] M6d.1 RECEBER tela ESCRITO+APK OK (falta validar): snapshot pega tambem source SCREEN_SHARE; quem compartilha ganha um tile extra no grid (span 2 col, aspect 16:9, ScaleType.FitInside, label "(tela)"). Render do nativo vendo a tela que a web/outros compartilham.
    - [ ] M6d.2 TRANSMITIR tela: MediaProjection (dialogo do sistema) + foreground service tipo mediaProjection (Android 14) + setScreenShareEnabled 60fps/bitrate alto (o "gamer").
...
- 2026-06-18: M6d.1 RECEBER tela ESCRITO + APK OK. Snapshot agora pega SCREEN_SHARE; tile extra (span 2 col, 16:9, FitInside) pra quem compartilha. ScaleType e FitInside/Fill (Fit nao existe). Falta validar (web compartilha -> nativo ve). Depois M6d.2 (transmitir, MediaProjection).
