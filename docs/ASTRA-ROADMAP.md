...
- [~] M7 Design editorial-cosmico (paridade visual com o web; user pediu "copiar tudo do Capacitor, otimizar com Compose, nao se conter nas animacoes"). Accent = prata-estelar #d4d8e0 (NAO mais ambar; web migrou Umbra->Astra cosmic). Fatiado:
  ...
  - [~] Telas (em progresso): [x] Splash cosmica · [x] Login (hero): CosmicBackground + glow radial, wordmark serif, numeral romano "I." + marginalia, titulo serif, EditorialField (BasicTextField underline flush que acende no foco, label mono em cima, placeholder italic), botao accent uppercase, stagger via Reveal. · [ ] Home -> listas -> chat (bolhas slide-in + starlight sweep) -> call.
