...
- [~] M6 LiveKit Android (fatiado, valida cada slice com 2 contas):
  ...
  - [~] M6d screenshare (fatiado):
    ...
    - [~] M6d.2a TRANSMITIR tela ESCRITO+APK OK (falta validar): botao Tela -> dialogo MediaProjection (createScreenCaptureIntent, StartActivityForResult) -> setScreenShareEnabled(true, ScreenCaptureParams(resultData, notificationId, notification)). O LiveKit roda o PROPRIO foreground service (ScreenCaptureService type=mediaProjection, ja no manifesto da lib — confirmado no merge final). ControlBar virou 2 linhas (Mutar/Surdo + Camera/Tela) + Sair. Encoding ainda no default.
    - [ ] M6d.2b tuning 60fps "gamer": RoomOptions.screenShareTrackPublishDefaults (VideoEncoding maxFps 60 + bitrate alto, simulcast off) + screenShareTrackCaptureDefaults (maxFps 60). Setar no LiveKit.create do join.
...
- 2026-06-18: M6d.2a TRANSMITIR tela ESCRITO + APK OK. MediaProjection (createScreenCaptureIntent) -> setScreenShareEnabled(ScreenCaptureParams). LiveKit roda o proprio FGS (ScreenCaptureService type=mediaProjection da lib; confirmado MEDIA_PROJECTION+service no manifesto merged). Falta validar (nativo compartilha -> web ve). Depois M6d.2b (60fps) fecha o M6 -> app desktop nativo.
