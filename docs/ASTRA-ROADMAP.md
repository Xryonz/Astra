...
- [~] M6 LiveKit Android (fatiado, valida cada slice com 2 contas):
  ...
  - [~] M6b painel de participantes ESCRITO+APK OK (falta validar no device): lista quem esta na sala com avatar+nome real (resolvido via GET /api/servers/:id/members; identity do LiveKit = userId), anel ambar em quem fala (isSpeaking via eventos), icone mic mudo por pessoa, botoes Mutar/Silenciar(deafen)/Sair. Deafen = setVolume 0.0 nas RemoteAudioTrack + corta o proprio mic (estilo Discord). VoiceManager passou a expor participants[] e refaz snapshot em qualquer RoomEvent. serverId passa na rota call/ pra resolver nomes.
...
- 2026-06-18: M6b painel de participantes ESCRITO + APK assembleDebug OK. Avatar+nome real (via /members), anel ambar em quem fala, icone mic mudo, deafen (setVolume 0 nas tracks remotas + corta proprio mic). VoiceManager expoe participants[] e refaz snapshot a cada RoomEvent. Falta validar no device com 2 contas. Proximo: M6c (video de camera).
