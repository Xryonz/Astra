...
- [ ] M4 WebSocket+DMs (fatiado): M4a SocketManager (conecta+heartbeat+reauth) [VALIDADO: chip Online] · M4b lista de DMs (REST GET /api/dm, Coil avatar, nav Home->Lista) [VALIDADO: vazio OK] · M4c chat DM (historico REST + join_dm/new_dm + enviar POST, dedupe por id, abrir por @username, bolhas mine/other, userId persistido no login) [escrito+compila, validar runtime]
  - Socket auto-recupera token expirado: no connect_error de auth, refresca via RefreshApi e reconecta (server revalida token em TODA conexao). Bug "offline apos uns min" resolvido.
...
- 2026-06-18: M4c chat DM escrito+compila — abrir conversa por @username (POST /api/dm/open), historico (GET messages), realtime via SocketManager (new_dm/dm_deleted como SharedFlow, join/leave), enviar (POST), dedupe por id, bolhas mine/other (userId persistido no DataStore no login). Falta validar runtime.
