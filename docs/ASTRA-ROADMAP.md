...
- [~] M6 LiveKit Android (fatiado, valida cada slice com 2 contas):
  ...
  - [~] M6d screenshare (fatiado):
    ...
    - [~] M6d.2b tuning 60fps "gamer" ESCRITO+APK OK (falta validar): RoomOptions.screenShareTrackPublishDefaults = VideoTrackPublishDefaults(VideoEncoding(8Mbps, 60fps), simulcast=false) no LiveKit.create do join. Codec mantido VP8 (default) de proposito: VP9/AV1 derrubam frame no encode mobile. VideoEncoding e (maxBitrate, maxFps).
...
- 2026-06-18: M6d.2b tuning 60fps ESCRITO + APK OK. RoomOptions.screenShareTrackPublishDefaults (VideoEncoding 8Mbps/60fps, simulcast off) no LiveKit.create. VP8 mantido (VP9/AV1 derrubam frame no mobile). Falta validar. Se OK = M6 INTEIRO fechado (voz/video/tela nativos) -> proximo grande marco: app desktop nativo (Compose Multiplatform).
