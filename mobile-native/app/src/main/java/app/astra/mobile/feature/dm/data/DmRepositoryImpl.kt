...
class DmRepositoryImpl @Inject constructor(
    ...
    override suspend fun close(conversationId: String): Result<Unit> = try {
        dmApi.close(conversationId)
        Result.success(Unit)
    } catch (e: CancellationException) {
        throw e
    } catch (e: Exception) {
        Result.failure(ApiException("Falha ao fechar a conversa"))
    }
...
private fun DmMessageDto.toDomain(currentUserId: String?) = DmMessage(
    ...
    content = content,
    authorId = senderId,
    authorName = author?.displayName ?: author?.username ?: "Alguém",
